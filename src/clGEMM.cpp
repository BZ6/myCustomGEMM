...
// Configure the thread/work-group dimensions of the myGEMM kernel
    ...
    // TODO: замер каждого запуска ядра
    profile_t timersRun[NUM_RUNS];
...
// Remove padded zeroes from the larger output
        ...
        // TODO: замер каждого запуска ядра 
        timersRun[r].t += timer() - startTimeRun;
    ...
    // TODO: замер каждого запуска ядра
    for (int t = 0; t < NUM_RUNS; t++) {
        printf("%9s, %d, %d, %s, %d, %6.6lf\n",
            "myGEMM.cl", KERNEL, TS, COMPILER_OPTIONS, N, wtime(timersRun[t]));
    }
